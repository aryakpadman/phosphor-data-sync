...
DataWatcher::DataWatcher(sdbusplus::async::context& ctx, const int inotifyFlags,
                         ...
                         std::optional<std::pair<excludeListSet, excludeListStr>> excludeList,
                         std::optional<std::unordered_set<fs::path>> includeList) :
    _inotifyFlags(inotifyFlags), _eventMasksToWatch(eventMasksToWatch),
    _dataPathToWatch(dataPathToWatch), _excludeList(excludeList), _includeList(includeList),
...
bool DataWatcher::isPathExcluded(const fs::path& path)
{
    ...
    if (std::ranges::any_of(_excludeList.value().first,
    ...
    DataWatcher::processCreate(const EventInfo& receivedEventInfo)
{
    ...
    if ((std::get<2>(receivedEventInfo) & IN_ISDIR) != 0)
    {
        ...
        else if (_dataPathToWatch.string().starts_with(
