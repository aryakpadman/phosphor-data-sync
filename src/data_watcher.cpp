...
std::map<Cookie, DataOperation> DataWatcher::_savedDataOps;
    ...
    DataWatcher::processMovedFrom(const EventInfo& receivedEventInfo)
{
    ...
    {
        _savedDataOps.emplace(std::get<3>(receivedEventInfo),
    ...
    DataWatcher::processMovedTo(const EventInfo& receivedEventInfo)
{
    ...
    if (absCopiedPath.string().starts_with(_dataPathToWatch.string()))
    {
        ...
        if (_savedDataOps.contains(cookie))
        {
            if ((_savedDataOps[cookie].first.filename().string())
                    .starts_with("."))
            {
            ...
            else
            {
                lg2::debug("[{OLDPATH}] renamed/moved to [{NEWPATH}]",
                           "OLDPATH", _savedDataOps.at(cookie).first, "NEWPATH",
                           absCopiedPath);
                _savedDataOps.erase(cookie);
            }
        }
