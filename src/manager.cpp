                          ...
                          const std::string& srcPath, std::string& cmd)
{
    ...
    if (mode == RsyncMode::Sync)
    {
        ...
        cmd.append(
            ...
            " --update --relative --delete --delete-missing-args --stats ");
    ...
    else if (mode == RsyncMode::Notify)
    {
        ...
        cmd.append("rsync --compress --recursive --perms --group --owner "
                   "--times --atimes --update --remove-source-files"s);
    }
        ...
        const config::DataSyncConfig& dataSyncCfg, const std::string& srcPath)
{
    ...
    co_return co_await executor.execCmd(notifyCmd);
}
                      ...
                      fs::path srcPath)
{
             ...
             (utility::rsync::getTransferredBytes(result.second) != 0))
    {
        ...
        // initiate sibling notification
        auto result = co_await triggerSiblingNotification(dataSyncCfg, srcPath);
        if (result.first != 0)
        {
            // TODO : Need to retry if notify command fails.
            lg2::error(
                "Failed to send notify request for the configured path [{PATH}], "
        ...
        {
            lg2::debug("Successfully send notify request for the configured "
                       "path : [{PATH}]",
