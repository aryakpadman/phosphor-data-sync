...
sibling. In a normal full sync, `--delete` and `--delete-missing-args` are used
so that files deleted on the source side are also removed on the destination,
...
For example: Let's consider syncing of PELs which are configured for
...
sequenceDiagram
    ...
    par BMC0 pre-fullSync
        ...
        BMC0->>BMC0: Remove files from BMC0 which are missing in BMC1 <br/> only if file's modified time < SyncDisableTime <br/> {PEL2 Removed}
        BMC0->>BMC1: Full sync (with --update &--delete-missing-args) <br/> {PEL5 copies to BMC1}
    and BMC1 pre-fullSync
        ...
        BMC1->>BMC1: Remove files from BMC1 which are missing in BMC0 <br/> only if file's modified time < SyncDisableTime <br/> {PEL3 Removed}
        BMC1->>BMC0: Full sync (with --update &--delete-missing-args) <br/> {PEL4 copies to BMC0}
    end
    ...
    Note over BMC0,BMC1: Resume Normal operation (--update, --delete, --delete-missing-args)
```
